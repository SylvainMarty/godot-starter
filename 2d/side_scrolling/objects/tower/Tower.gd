...
@export var tower_id: LevelManager.TowerId
...
func _ready():
	if LevelManager.is_surface_level():
		can_climb_down = true
	else:
		tower_sprite.modulate = '#ff5e36'
		if not LevelManager.is_last_level():
			can_climb_up = true
			can_climb_down = true
		else:
			can_climb_up = true
...
func _process(delta):
	...
	if can_climb_up and Input.is_action_pressed("climb_up"):
		LevelManager.climb_up(tower_id)
		climbing = true
		return
	if can_climb_down and Input.is_action_pressed("climb_down"):
		LevelManager.climb_down(tower_id)
		climbing = true
		return
